const getPostData = () => {
  const content = ['1. React 也是一個JavaScript函式庫只是專門用於構建UI，所以他一樣需要一個能夠運作js代碼的運行環境，而NodeJS就有這樣的用途，除了提供運行環境外，還有提供函式庫的管理(npm)',
    '2. 我將屬於貼文的Styled-component獨立出一個模塊讓整個專案更有結構性、CSS-in-JS是一種模組化的表現，讓每一個component都伴隨其style這樣能更好維護與理解尤其是在重構的時候，而他最主要就是解決了傳統CSS常見的問題比如：全域命名空間的污染、特定性衝突以及缺乏封裝性。',
